SmartAgent::Tool.define :smart_search do
  ...
  tool_proc do
    SmartResearchCLI.logger.info "call_tool(:get_research_topics)"
    topics = call_tool(:get_research_topics)
    SmartResearchCLI.logger.info "call_tool(:search)"
    result = call_tool(:search, { "query" => input_params["q"], "num" => 10 })
    SmartResearchCLI.logger.info "call_tool(:get_topic)"
    ...
    SmartResearchCLI.logger.info "call_tool(:create_research_topic)"
    ...
    search_result.each do |sr|
      ...
      if SourceDocument.where(url: sr["link"]).empty?
        begin
          #SmartResearchCLI.logger.info "call_tool(:scrape), url is #{sr["link"]}"
          SmartResearchCLI.logger.info "fake call_tool(:scrape), url is #{sr["link"]}"
          # text = call_tool(:scrape, { "url" => sr["link"] })
          SmartResearchCLI.logger.info "call_tool(:create_or_update_source_document_and_section)"
          ...
          SmartResearchCLI.logger.info "文章抓取失败"
          SmartResearchCLI.logger.info e.to_s
        end
      end
