SmartAgent::Tool.define :smart_search do
  ...
  tool_proc do
    SmartAgent.logger.info "call_tool(:get_research_topics)"
    topics = call_tool(:get_research_topics)
    SmartAgent.logger.info "call_tool(:search)"
    result = call_tool(:search, { "query" => input_params["q"], "num" => 10 })
    SmartAgent.logger.info "call_tool(:get_topic)"
    ...
    SmartAgent.logger.info "call_tool(:create_research_topic)"
    ...
    search_result.each do |sr|
      ...
      if SourceDocument.where(url: sr["link"]).empty?
        begin
          #SmartAgent.logger.info "call_tool(:scrape), url is #{sr["link"]}"
          SmartAgent.logger.info "fake call_tool(:scrape), url is #{sr["link"]}"
          # text = call_tool(:scrape, { "url" => sr["link"] })
          SmartAgent.logger.info "call_tool(:create_or_update_source_document_and_section)"
          ...
          SmartAgent.logger.info "文章抓取失败"
          SmartAgent.logger.info e.to_s
        end
      end
